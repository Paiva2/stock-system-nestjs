...
describe("Filter category by id service", () => {
  ...
  it("should filter an category by id", async () => {
    const newCategory = await sut.create(user.id, "Fruits");

    const categoryDeleted = await sut.filterCategoryById(user.id, newCategory.id);
  ...
  it("should not filter an category by id without correctly provided parameters", async () => {
    ...
    await expect(() => {
      return sut.filterCategoryById("", newCategory.id);
    }).rejects.toEqual(new BadRequestException("Invalid user id."));

    await expect(() => {
      return sut.filterCategoryById(user.id, "");
  ...
  it("should not filter an category by id if user doesn't exists", async () => {
    await expect(() => {
      return sut.filterCategoryById("Inexistent user", "Any category id");
  ...
  it("should not filter an category by id if user category't exists", async () => {
    await expect(() => {
      return sut.filterCategoryById(user.id, "Inexistent caregory");
